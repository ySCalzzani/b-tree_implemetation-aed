...
file.read ((char*)(&node), sizeof(BTreeNode<M>));     // 1 registro
  \end{lstlisting}
  \footnotesize Conferência: arquivo de $M{=}3$/1000 chaves = 35\,820 bytes
  $\div$ 36 bytes-por-nó $=$ 995 registros exatos.
\end{frame}
...
\begin{frame}[fragile]{Como testamos: oráculo por \emph{fixtures}}
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}{Métricas Utilizadas (Slide 5)}
  \begin{description}
    ...
    \item[tempo\_cpu / tempo\_io] Tempo de CPU \emph{separado} do tempo de espera
      de I/O (\texttt{tempo\_cpu = total $-$ tempo\_io}).
...
% ===== Perguntas do enunciado -> respostas ==================================
\begin{frame}{Perguntas do enunciado $\to$ onde respondemos}
  \centering\footnotesize
  \begin{tabular}{p{6.3cm} l}
    \toprule
    \textbf{Pergunta experimental} & \textbf{Resposta} \\
    \midrule
    Como a ordem $M$ afeta os acessos a disco? & fig.\ \emph{reads}, tabela \\
    Conjuntos de $10^3$ a $10^6$, Seq e Aleatório & 70 execuções (varredura) \\
    Nº médio de acessos por operação & \texttt{avg\_reads}, fig.\ \emph{reads} \\
    Ocupação do arquivo (com e \alert{sem} reuso) & fig.\ \emph{reuso}, fig.\ \emph{ocupação} \\
    Tempo de execução: CPU \emph{vs.} I/O & fig.\ \emph{trade-off} \\
    Inserção Sequencial \emph{vs.} Aleatória & fig.\ \emph{reads}/\emph{ocupação} \\
    Altura da árvore \emph{vs.} $M$ e \emph{vs.} $N$ & fig.\ \emph{altura} \\
    Existe uma ordem $M$ ótima? & fig.\ \emph{trade-off} ($\sim M{=}100$--$500$) \\
    \bottomrule
  \end{tabular}
  \vspace{0.5em}

  \footnotesize Os gráficos a seguir são gerados de
  \texttt{data/resultados\_experimentos.csv} por
  \texttt{analysis/gerar\_graficos.py} (\texttt{make graficos}).
\end{frame}
...
% ===== Grafico: trade-off ===================================================
\begin{frame}{Trade-off CPU {\small$\times$} I/O: a ordem ótima}
  \centering
  \includegraphics[height=0.74\textheight]{fig_tradeoff.png}

  \footnotesize $M$ pequeno é \alert{limitado por I/O} (mais acessos); $M$ muito
  grande gasta \alert{CPU} reorganizando nós grandes. Ótimo em $\sim M{=}100$--$500$.
\end{frame}
...
\begin{frame}{Utilização de IA (Slide 7)}
  ...
  \begin{description}\footnotesize
    \item[Revisão de código] Revisão crítica que apontou casos de borda ---
      ...
      existia ($\to$ \texttt{throw} em \texttt{readNode}/\texttt{writeNode} e
      criação de \texttt{tmp/}).
    \item[Geração de casos de teste] Sugestão de \emph{fixtures} e cenários
      (split em cascata, \emph{borrow}/\emph{merge}, colapso da raiz, no-ops)
  ...
  \end{description}
  \vspace{0.2em}
  \footnotesize\color{btgray} Decisões de projeto, implementação e análise
  permaneceram do grupo; a IA acelerou estudo, revisão e redação.
\end{frame}

% ===== Slide 8: Dificuldades + Vantagens/Desvantagens =======================
\begin{frame}{Dificuldades, Vantagens e Desvantagens (Slide 8)}
  \begin{columns}[T,onlytextwidth]
...
% ===== Slide 9: Aplicacoes Praticas =========================================
\begin{frame}{Aplicações Práticas (Slide 9)}
  \begin{description}
    \item[Bancos de dados relacionais] MySQL InnoDB e PostgreSQL usam Árvores B+
      nos índices para localizar registros em $O(\log n)$ acessos a disco.
    \item[Sistemas de arquivos] NTFS, APFS e ext4 usam Árvores B/B+ para
      diretórios e localização de blocos.
    \item[NoSQL / embarcado] LevelDB, RocksDB e SQLite.
  \end{description}
  \vspace{0.3em}
  \footnotesize\alert{Aplicação que adotaríamos:} o índice de um SGBD próprio ---
  exatamente o cenário deste trabalho (registros de tamanho fixo, 1 nó por acesso).
\end{frame}

% ===== Slide 10: Conclusao e Referencias ====================================
\begin{frame}{Conclusão e Referências (Slide 10)}
  \begin{itemize}
    ...
    \item \textbf{CPU vs I/O:} $M$ pequeno é \alert{limitado por I/O} e o mais
      lento ($M{=}3$/$10^6 \approx 16,9$\,s, $t_{io}\,14,2$\,s); ótimo em
      $\sim$\,$M{=}100$--$500$; $M$ muito grande volta a subir (CPU).
    \item \textbf{Reuso de nós economiza $\approx$33\%} de ocupação do arquivo.
    \item \textbf{Robustez:} contador de I/O agora falha alto (\texttt{throw} se
      o arquivo não abre); \texttt{tmp/} é criado antes de medir.
